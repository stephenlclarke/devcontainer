...
flowchart LR
    ...
    Broker -->|Commit source record| Journal[SQLite output journal]
    Broker -->|Then publish| Live[Live subscriptions]
    Journal --> History[Bounded history cursor]
    History --> Reader[Demand-driven HTTP reader]
    Live --> Reader
```

Under one broker lock, a frame is committed before live publication. The same lock captures the finite history cutoff and registers the live subscription, eliminating gaps, duplicates and competing stdout/stderr publication order. Capture is prepared exactly once per process generation before native bootstrap. Restart preserves earlier complete records for the same immutable Docker ID/native creation identity; generation replacement waits for reader/control joins and durable completion publication. A cancelled attachment does not stop capture or manufacture a native exit result.

Schema 5 adds `runtime_output_journals` and `runtime_output_frames`. Foreign keys delete output with its exact metadata identity. Each writer and reader owns a separate full-mutex SQLite connection; both output and lifecycle connections use a one-second contention bound. WAL transactions use full durability, never await an asynchronous client, and never remain open between replay reads. Missing records, failed writes, failed completion, changed identity, interrupted ownership and removal during replay are errors, not clean EOF. A shared failure status also invalidates already-open readers when disk/lock failure prevents updating the durable marker.

Current development limits are 64 KiB per source frame, 1 GiB of retained payload plus at most 1,048,576 records per container, and five minutes (or an earlier request deadline) for finite replay. Index/WAL overhead is additional to the payload budget. Exhaustion fails explicitly without deleting or truncating previous records. Live subscribers still have a 256-frame bound; replaying old history does not add another HTTP queue, but a slow client can still exhaust its live subscription. These limits and interrupted-capture refusal are visible compatibility constraints, not claims of unlimited Docker log parity. General `/logs`, imported/external history, live fault tests and the complete foreground oracle remain unqualified by the component evidence.
