...
`ManagedNetworkHostsStore` now implements the backing-file primitive, but is not yet connected to creation or lifecycle calls. It publishes an operation-UUID directory only after the file and inode/device provenance are durable; unpublished `.alloc` directories are never native mount sources. Verified removal first retires the directory, then removes only known members through directory descriptors. Recovery retains unexpected members, changed complete provenance and unsafe paths. The shared parent contains only the hosts file intended for a read-only guest mount; host updates retain its inode and explicitly set permissions for non-root readers. These local storage tests do not prove startup DNS or native runtime recovery.
