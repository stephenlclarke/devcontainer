...
class DevcontainerComposeRecoveryTests(DevcontainerRecoveryTests):
    fixture_name = "C01-compose-service"

    def patches(self):
        stack = super().patches()
        stack.enter_context(patch("devcontainer_compose_reference.DevcontainerComposeReference", return_value=self.fixture))
        return stack


class DevcontainerDependenciesRecoveryTests(DevcontainerRecoveryTests):
    fixture_name = "C02-compose-dependencies"

    def patches(self):
        stack = super().patches()
        stack.enter_context(patch("devcontainer_dependencies_reference.DevcontainerDependenciesReference", return_value=self.fixture))
        return stack
