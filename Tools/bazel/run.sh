    ...
    if [[ ( "$command" == coverage || "$command" == test ) && "$status" == 0 && "$validation" == 0 ]]; then
        ...
        if [[ -n "$suite" ]]; then
            ...
            if [[ "$suite" == source && ( -f "$repo/Tools/bazel/evidence-policy.json" || "$repo/Tools/bazel" != "$TOOL_DIRECTORY" ) ]]; then
                evidence_args+=(--policy "$repo/Tools/bazel/evidence-policy.json")
            fi
            /usr/bin/python3 "$TOOL_DIRECTORY/check_evidence.py" ${evidence_args[@]+"${evidence_args[@]}"} --suite "$suite" --profile "$profile" \
...
main() {
    ...
    if [[ "$command" == acquire-releases || "$command" == prepare-releases || "$command" == prepare-guest-images || "$command" == prepare-docker-cli || "$command" == prepare-devcontainers-cli ]]; then
        if [[ "$command" == prepare-guest-images ]]; then
            [[ $# -ge 1 ]] || { error "$command requires LOCK [--offline] [--source-archive NAME=ABSOLUTE_PATH]."; return 2; }
        else
        ...
        if [[ "$command" == prepare-devcontainers-cli ]]; then
            release_helper=prepare_devcontainers_cli.py
        fi
