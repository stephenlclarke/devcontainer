...
def _candidate_archive_impl(ctx):
    ...
    ctx.actions.write(manifest, json.encode({
        ...
        "reference": {
            "node": ctx.file.reference_node.path,
            "nodeLicense": ctx.file.reference_node_license.path,
            "lock": ctx.file.reference_lock.path,
            "cli": {file.path.removeprefix(file.owner.workspace_root + "/"): file.path for file in ctx.files.reference_cli},
        },
    }))
    ctx.actions.run(
...
candidate_archive = rule(
    ...
    attrs = {
        ...
        "reference_node": attr.label(allow_single_file = True, mandatory = True),
        "reference_node_license": attr.label(allow_single_file = True, mandatory = True),
