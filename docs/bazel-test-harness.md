...
`service_switch.py` adds the next controlled-runtime primitive, currently exercised only with simulated launchd services and **not wired into the live target**. It snapshots explicitly authorised service definitions before removal, journals every mutation, refuses unknown workloads/foreign replacements, and restores the original unchanged definitions after partial removal or interrupted registration. Its recovery constructor requires authenticated private journal inputs; the journal persistence and operator recovery entry point still need integration. Snapshot bytes may contain service environment secrets and must stay in private internal storage, never a published test artifact. The caller must hold the shared runtime lease and prove worker quiescence/host idleness before using it. No real service has been replaced by these tests.

Sixteen simulated switch tests cover successful restoration, unknown workloads, changed/foreign definitions, interrupted removal/registration, a fresh recovery object and parent-driven plugin removal. Standard-library trace measured 94% focused line coverage for this primitive on 17 September 2026; this is diagnostic coverage, not an aggregate product or SonarQube result. The combined native harness passes 69 cases (invocation `1c8f87c6-6afc-4147-a96c-3f71c0073662`), with the unchanged 14-case preparation target reused from its passing result. Repository lint also passes after correcting the Swift regression's line length; a new exact-head remote analysis remains necessary.
