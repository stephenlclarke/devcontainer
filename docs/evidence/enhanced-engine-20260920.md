...
### Instrumented bootstrap checks and authorization interruption

Five focused bootstrap test functions now cover process transport failures without retries, disconnect after failure, output-only EOF policy, unknown-signal encoding and the signed exit-code boundary. The stock SDK's public process protocol has no explicit disconnect method, so that assertion is enhanced-only. Initial stock compilation failure `21701d25-f57c-4a65-a35e-5de5dd035690` remains retained. Corrected stock invocation `4670825d-2743-401f-98b6-6371fc9f2d92` and enhanced invocation `48dbf695-d394-455d-9baf-45302218a998` pass with Bazel-reported elapsed times of 28.650 and 32.211 seconds respectively, including compilation; these are not quiet benchmarks. Measured bootstrap-source line coverage is 55/64 (85.9%) stock and 58/64 (90.6%) enhanced. The default live XPC connection is not exercised by these injected-transport tests; stock also cannot call the enhanced-only disconnect member. This focused stock shortfall remains explicit, not excluded or treated as aggregate product coverage. Complete independent test review is clean. Product source and the existing prepared archive are unchanged.

The optimized combined-fix package uses Container source `fc83384f63686fbcdd3c94f8bb19b1ea9f8c46ec`, devcontainer source `3d859619b3086827b80e4e97498b43ccbc228ac5` (package invocation `beb76b65-6ced-42ef-b4de-00228fb699fc`) and corrected guest `e45a379`. Its two live admissions time out during setup before any guest boot or parity operation: first the API server requests removable-volume access, then, after an observed authorization response, the separate network helper requests access. Both automatically restore original services and remove all owned scratch resources. Their immutable records are:

| Case ID | Result seal | Setup seconds | Operation seconds | Cleanup seconds |
| --- | --- | ---: | ---: | ---: |
| `cbad741418cf8107271508cf9abafcbee8129f6d56ec2fb80f22e346c7fb97f0` | `542cb616e80ff5cbdce0a40f0f84a9bde1a2acb88f46cc43f61e637b52355d81` | 11.567225000 | 0 | 1.535010500 |
| `cdde5d2cd6872f6eff5adaeb64a025d642f8a0c2eed264ed2a3a188ad99d8873` | `2b210089f87359ee52825e9594d74e017f0e499f5eb1d66908bf533d458ddc4f` | 11.019573292 | 0 | 1.582797958 |

At the operator's request, further prompt-producing runtime attempts are paused. Signing identity alone did not preserve approval across changed bare-executable paths. Stable executable locations, explicit one-time enrollment and bounded authorization preflight remain required for unattended qualification; no consent automation, security bypass or timeout relaxation is implemented. This is an authorization blocker, not an observed failure of the combined EOF correction.

The Linux coverage build separately exposed a missing `libclang_rt.profile-aarch64.a` in the installed Swift 6.3 static SDK. A diagnostic archive built from the matching [Swift LLVM source](https://github.com/swiftlang/llvm-project/tree/944ef77b000119f9dfa308fb7171dd0772bc3fc2/compiler-rt/lib/profile) allows the existing instrumented Linux component objects to link without modifying that SDK. The archive SHA-256 is `b895ffc4f04b882cdae0212b7e39489461c2ce210304ac00a3bdc33406b1dc22`; the resulting static ARM64 ELF SHA-256 is `61d40712ba6044f71090f0fcb8fd1598297c5e1ecfa649814be5ef00b0a8616f`. Source, library, binary and exact link plan are retained privately. The executable has not run: no raw profile, Linux coverage result or Sonar recovery is claimed.
