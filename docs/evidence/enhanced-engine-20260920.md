...
These are focused functional observations, not quiet benchmarks, complete three-lane parity, published-release comparisons or stable-release qualification. Enhanced E03 exec transfer remains unresolved. A subsequent instrumented E07 run proves the timeout is in combined duplex transfer after startup history, not initial history replay. A separate native CLI control reproduces a backpressure-dependent stall without devcontainer carrying the transfer. The exact lower-level cause and whether it explains both E03 and E07 remain unproved.
...
Next proof: isolate the native/guest relay backpressure failure before choosing a correction. Source inspection identifies blocking writes on the shared guest I/O event loop as a candidate, not yet a verified cause. That IOPair source is identical between the admitted enhanced revision and stock Containerization 0.45; passing ordinary stock tests do not disprove a backpressure-sensitive defect. Do not increase deadlines, throttle tests to hide the stall or rerun E03 unchanged. Any required lower-fork correction must use its own coordinated pull request and retain stock Apple compatibility proof.
