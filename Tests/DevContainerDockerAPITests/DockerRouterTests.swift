...
func `health registry observes intervals retries start periods and resets`() async {
    ...
    guard case .check = await registry.decision(
        ...
        Issue.record("first health observation must execute")
        return
    }
    ...
    guard case let .cached(cached) = await registry.decision(
        id: "fixture",
        startedAt: start,
        healthcheck: check,
        now: start.addingTimeInterval(0.2)
    ) else {
        Issue.record("health checks inside the interval must be cached")
        return
    }
    #expect(cached == first)
    guard case .check = await registry.decision(
        ...
        Issue.record("health check must run after its interval")
        return
    }
    ...
    guard case .check = await registry.decision(
        ...
        Issue.record("reset health state must execute a new observation")
        return
    }
    await registry.remove(id: "fixture")
}

private func bytes(_ response: DockerHTTPResponse) throws -> Data {
...
private func streamBytes(_ response: DockerHTTPResponse) async throws -> Data {
