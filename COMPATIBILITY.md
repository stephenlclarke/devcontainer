...
C02 health-policy projection is an unreleased, component-tested candidate. It requires a matching Compose stock adapter and an explicitly negotiated gateway capability; unknown gateways or malformed policy fail rather than silently losing health checks. The evaluator uses actual guest probes, but remains inspect-driven and rejects explicit `start_interval`. Corrected live stock qualification is pending and enhanced/release evidence is incomplete. This does not expand the stable compatibility matrix. See [C02 evidence and limits](docs/bazel-test-harness.md#c02-dependency-health-and-service-selection).
