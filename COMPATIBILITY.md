...
Draft creation recovery uses current state schema 5 and requires `RuntimeCreationStore` for direct native creation. It retains uncertain operations and blocks launch/exec/archive/rename of the affected incarnation without unsafe automatic deletion. Schema-2/3/4 migration is supported; older binaries cannot open the upgraded state. Operator reconciliation and live interrupted-create proof are still outstanding, so this is not a completed recovery/parity claim. [Native creation recovery](DESIGN.md#native-creation-recovery) specifies the exact scope and rollback rule.

Unreleased native `attach?logs=1` uses source-aware schema-5 capture for engine-owned process generations, preserving history across clean restarts/reopen and atomically joining live output. It does not relabel merged native logs. The current 1-GiB payload/1,048,576-record retention budget, five-minute replay deadline, 256-frame live buffer and refusal of interrupted or external capture are explicit development constraints. General `/logs` and complete Docker foreground parity remain unqualified. See [source-aware output history](DESIGN.md#source-aware-output-history).
