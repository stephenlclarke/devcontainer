...
C02 dependency health/DNS/runServices is the active unreleased contract. The real Docker reference passes all three unchanged observations and cleanup; stock Apple startup fails because the native Compose launch forwards unsupported health-check flags. Exact case recovery removed the unattached test network, stopped the captured Engine and restored prior services without changing the failed result. Fifteen focused socket tests and 841 affected Bazel harness tests pass; new-adapter focused line coverage is 93.71%. Native qualification requires a semantic health-check projection fix. No existing product binary or reference release was rebuilt for these runs; exact evidence is in the repository's `docs/bazel-test-harness.md`.
