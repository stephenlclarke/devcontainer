...
The active unreleased C01 native harness accepts a separately prepared, profile-matched Compose candidate through `COMPOSE_CANDIDATE_INVOCATION`, alongside the devcontainer candidate. Its first stock live run created the service and completed post-create, but failed verification; recovery restored previous services and retained the failed result. A vendor-qualified native short-version response now prevents the official CLI from interpreting native 0.15.1 as legacy Docker Compose and changing project names. Network/image projection gaps and successful live qualification remain outstanding. The repository's `docs/bazel-test-harness.md` records exact candidate identities, results, recovery and invocation examples.
