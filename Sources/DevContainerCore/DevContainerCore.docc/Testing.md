...
The Docker E04 lane now passes positive build, failed-command execution and cleanup. The stock Apple lane has not yet passed: its first attempt provisioned the released builder but stopped before building because image inspection omitted the repository manifest digest. The candidate fixes that metadata and uses explicit named-reference cleanup only inside the exclusive private Apple store, with tag/config/manifest checks and garbage collection disabled. This is not general image-ID deletion parity; digest-addressed public mutations remain refused. Functional-run timings are retained separately from quiet comparative benchmarks.
