...
D04 lifecycle hooks is active: the checked-in fixture must observe host initialization and `onCreate,updateContent,postCreate,postStart,postAttach` in that exact order. The native Bazel adapters preserve pinned image ownership and bounded normal cleanup; no hook output is synthesized or normalized. Docker supports cleanup-only recovery after verified command completion; interrupted native phases still require explicit reconciliation and stay quarantined. Reference and candidate live execution remain required. Repeated reopen/restart behavior and hook-failure matrices are outside this fixture.
