...
C01 Compose-service configuration is the active harness contract. The new reference adapter retains the original Compose file and probe, uses the pinned published Compose executable, and verifies service and default-network ownership before exact-resource cleanup. Its component tests include interrupted commands, failed probes, foreign resources and reappearance after completed cleanup. Live reference and Docker-free native-provider qualification remain outstanding; native C01 admission currently refuses before runtime mutation.
