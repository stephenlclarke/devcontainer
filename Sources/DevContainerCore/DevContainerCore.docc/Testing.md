...
The active unreleased C01 native harness accepts a separately prepared, profile-matched Compose candidate through `COMPOSE_CANDIDATE_INVOCATION`, alongside the devcontainer candidate. Both archives are authenticated and reused without rebuilding; config/state paths are isolated and service/network cleanup remains identity-bound. Preparation and runtime components pass, but this is not live native parity or release qualification. The repository's `docs/bazel-test-harness.md` records exact candidate identities, results and invocation examples.
