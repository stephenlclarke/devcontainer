...
C02 dependency health/DNS/runServices is the active unreleased contract. Docker passes. Stock invocation `1dca60f5` now starts all three services and verifies actual database health through the corrected policy bridge, but the unchanged guest probe cannot resolve the `database` and `helper` service names. Cleanup passes with zero owned resources and recovery is clear. Network-host reconciliation for externally launched Compose services is the remaining observed gap; no C02 pass or stable support expansion is claimed. Exact sources, packages, component results, failed-run evidence and timings are in `docs/bazel-test-harness.md`.
