...
C02 dependency health/DNS/runServices is the active unreleased contract. Docker passes the unchanged fixture; the initial stock failure on unsupported health flags was safely recovered. Matching candidate changes now negotiate and validate a bounded requested-policy label, then derive health from actual guest probes with stale-completion protection. Complete affected component targets pass (155 Apple-runtime, 79 Engine-API and 12 provider functions); full source review is clean. Corrected live stock qualification is pending, and the bridge remains inspect-driven rather than an autonomous health scheduler. Exact component, failed-run and Docker evidence is in the repository's `docs/bazel-test-harness.md`; no stable support claim changes.
