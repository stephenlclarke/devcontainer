...
C02 dependency health/DNS/runServices is the active unreleased contract. Docker passes. Stock invocation `1dca60f5` starts all three services and verifies actual database health through the corrected policy bridge, but the unchanged guest probe cannot resolve the `database` and `helper` service names. Cleanup passes with zero owned resources and recovery is clear. The development branch now implements validated service aliases and scoped, restart-aware, cancellation-safe running-state reconciliation. Startup-time name resolution still requires the managed-file creation/start boundary and live qualification; no C02 pass or stable support expansion is claimed. Exact sources, packages, component results, failed-run evidence and timings are in `docs/bazel-test-harness.md`.
