...
D07 reuse/rebuild/cleanup passes its six original assertions on the pinned Docker reference (`be380dd2-e11c-44d3-8297-56b03212fb23`). Stock's first run (`f3177942-1ce6-460f-b395-93d14519ec00`) exposes missing named-volume frontend support before container creation. The candidate now projects named mounts through the existing native API; tests cover invalid names and real executable/private-socket requests. Corrected native live proof remains required. Failed evidence is retained, and no release or quiet paired benchmark is inferred from these functional runs.
