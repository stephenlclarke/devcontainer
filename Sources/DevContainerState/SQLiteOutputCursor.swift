...
final class SQLiteOutputCursor: @unchecked Sendable {
    ...
    private init(
        ...
        captureStatus: SQLiteOutputCaptureStatus?, logs: Bool
    ) throws {
        ...
        self.logs = logs
        cutoff = try Self.boundary(connection, snapshot: snapshot, generation: generation, logs: logs)
    }

    static func history(
        path: URL, snapshot: ContainerSnapshot, generation: String?, context: RuntimeRequestContext,
        captureStatus: SQLiteOutputCaptureStatus? = nil, logs: Bool = false
        ...
        let cursor = try SQLiteOutputCursor(
            ...
            captureStatus: captureStatus, logs: logs
        )
    ...
    private func next() throws -> RuntimeIOFrame? {
        try connection.synchronized {
            ...
            let frame = try connection.transaction(writing: false) {
                ...
                let current = try Self.boundary(connection, snapshot: snapshot, allowActive: true, logs: logs)
    ...
    private func readFrame() throws -> RuntimeIOFrame {
        ...
        SELECT channel, payload FROM \(logs ? "runtime_output_logs" : "runtime_output_frames")
        WHERE docker_id = ? AND sequence = ?
        """) { statement in
    ...
    static func boundary(
        ...
        generation: String? = nil, allowActive: Bool = false, logs: Bool = false
    ) throws -> Int64 {
        try connection.statement("""
        SELECT c.runtime_id, c.created_at, j.created_at, j.generation, j.complete,
               j.\(logs ? "log_sequence" : "last_sequence"), j.\(logs ? "log_bytes" : "stored_bytes"),
               j.log_version, j.generation_count, g.sources, g.ended
        FROM runtime_output_journals j JOIN runtime_containers c ON c.docker_id = j.docker_id
        LEFT JOIN runtime_output_generations g ON g.docker_id = j.docker_id AND g.generation = j.generation
        WHERE j.docker_id = ?
        """) { statement in
            try connection.bind(snapshot.dockerID.rawValue, to: statement, at: 1)
            guard sqlite3_step(statement) == SQLITE_ROW,
            ...
            if logs {
                let sourceMask = snapshot.spec.terminal ? 1 : 3
                let generations = sqlite3_column_int64(statement, 8)
                guard sqlite3_column_int(statement, 7) == 1,
                      generations > 0, generations <= SQLiteContainerOutputJournal.maximumStoredFrames,
                      sqlite3_column_int(statement, 9) == sourceMask,
                      complete != 1 || sqlite3_column_int(statement, 10) == sourceMask
                else { throw Self.failure("Versioned log history or natural source EOF is unavailable") }
            }
