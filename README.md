...
D02 Dockerfile configuration is the active fixture. The pinned official CLI passes all four unchanged observations and cleanup in the isolated Docker reference VM (`0686509c-bf9c-4ba3-a405-02fb0ebca041`, source `05cae9e`). It uses the legacy Engine build path, not an unpinned global Buildx builder. Setup/operation/cleanup took 17.683057833/1.214592000/2.948016291 seconds; these are functional observations, not quiet paired benchmarks. The candidate adapter now uses its private frontend and an admitted, isolated Apple builder with the same image-ancestry and lifecycle checks. Candidate live proof and enhanced prerequisites remain outstanding; reference execution does not create a Docker installation dependency for the product.
