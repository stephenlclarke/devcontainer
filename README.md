...
D07 reuse/rebuild/cleanup is the current contract. Docker passes immutable container identity, hook idempotence, persistent volume counters and ownership-bound removal. Stock now creates and reuses the named-volume container, but rebuild exposed a missing forced-removal command. The candidate frontend now implements that exact-ID command through the private native API; corrected live rebuild proof remains pending. See the [D07 contract](docs/bazel-test-harness.md#d07-reuse-and-cleanup). Component success does not qualify a release.
