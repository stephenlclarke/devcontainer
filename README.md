...
The next active fixture is D02 Dockerfile configuration. Its native harness can run the unchanged Dockerfile through the pinned official CLI in the isolated Docker reference VM with `CASE_FIXTURE=D02-dockerfile-config LANE=docker`. It explicitly uses the legacy Engine build path, not an unpinned global Buildx plugin, and checks build arguments, target, post-create output and workspace. Candidate D02 currently refuses before runtime mutation: its Docker-free build-command integration remains required. Reference execution is not a Docker installation dependency for the product.
