...
C01 Compose-service configuration is the active Bazel replacement contract. Its Docker reference passes the original service, hook and workspace assertions with private project/network ownership and verified cleanup. The first stock native run started the service and completed its post-create hook, but failed verification; its isolated resources were recovered without changing the failed result. Native short-version responses now identify `container-compose` explicitly so the official CLI cannot mistake its version for legacy Docker Compose and strip project-name hyphens. Network identity/membership and descriptor-verified digest-image spelling now have component-tested corrections with matching Compose changes; rebuilt-candidate live qualification remains outstanding. Run the admitted candidates with `COMPOSE_CANDIDATE_INVOCATION` alongside `CANDIDATE_INVOCATION`. See the [C01 contract](docs/bazel-test-harness.md#c01-compose-service).
