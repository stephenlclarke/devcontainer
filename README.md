...
The corrected stock run completes and cleans up automatically, but still fails the network-alias probe. The native Compose handoff now declares the adapter's existing creation-time alias support so configured aliases reach it; component regression proof is complete, with new packaged runtime proof still required.
