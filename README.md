...
The separate opt-in [service-process integration checks](docs/bazel-test-harness.md#bounded-service-http-probes) exercise startup, private provider access, a rejected recovery request and clean shutdown with a fake runtime command and a real disposable macOS Keychain in an isolated SSD HOME. They do not edit the login Keychain. Their test executable runs from a byte-identical SSD copy outside Bazel's incomplete test-bundle directory so strict code-identity validation remains enabled. These checks do not launch container VMs or establish released-binary performance. [Unattended authorization](docs/bazel-workflow.md#unattended-authorization-requirement) is mandatory: routine runs must fail with recoverable evidence rather than wait for approval; complete host/signing qualification remains pending.
