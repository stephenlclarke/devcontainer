...
The new Bazel E08 foreground fixture checks terminal resize, detach/reconnect of the same init process, acknowledged exit status and automatic removal. Its component tests and live Docker oracle pass. Stock testing exposed a missing `AutoRemove` inspection field and then an undeclared resize route; both adapter fixes still need complete stock/enhanced runtime qualification. See the [foreground test evidence and remaining requirements](docs/bazel-test-harness.md#foreground-init-attachment-development); this is not a stable-release support claim.
