...
The Bazel E14 real-PTY probe finds that both Docker and the stock candidate can reach the first guest size query before terminal dimensions arrive. Those failed stronger-than-reference assertions remain retained. Revised E14 now tests bounded post-start convergence and host-initiated resizing while preserving the first-query bytes. Its component tests pass; live qualification remains pending, and explicit Engine create-time sizing is still unsupported. See the [foreground evidence and limits](docs/bazel-test-harness.md#foreground-init-attachment-development).
