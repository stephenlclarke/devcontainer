...
C02 passes on the real Docker reference. The last live stock candidate starts all three services and verifies actual database health, but the guest probe fails service-name resolution. Cleanup passes with zero owned resources and recovery is clear. Running-state service aliases, restart-aware caching and scoped, cancellation-safe reconciliation are now implemented in the development branch; startup-time resolution through a managed file mount still requires implementation and live proof. C02 parity is not yet claimed. See the [C02 contract, timings and evidence](docs/bazel-test-harness.md#c02-dependency-health-and-service-selection).
