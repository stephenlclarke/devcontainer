...
The Bazel E14 real-PTY probe finds that both Docker and the stock candidate can reach the first guest size query before terminal dimensions arrive. Both fail the proposed pre-first-command size assertion and clean up without residue. This is retained oracle evidence, not a parity pass; the initial/ongoing terminal-size contract remains under review. See the [foreground evidence and limits](docs/bazel-test-harness.md#foreground-init-attachment-development).
