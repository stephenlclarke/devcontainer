...
C01 Compose-service configuration is the active Bazel replacement contract. Its Docker reference passes the original service, hook and workspace assertions with private project/network ownership and verified cleanup. The native harness now admits the separately retained stock Compose candidate, reuses both candidate archives without rebuilding and supplies isolated native config/state/runtime selection. Run it with `COMPOSE_CANDIDATE_INVOCATION` alongside `CANDIDATE_INVOCATION`; live native-provider proof remains outstanding. See the [C01 contract](docs/bazel-test-harness.md#c01-compose-service).
