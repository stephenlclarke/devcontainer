...
C01 Compose-service configuration is now the active Bazel replacement contract. Its reference adapter preserves the original service, hook and workspace assertions while checking private project/network ownership and recoverable cleanup. Component tests pass; live reference and Docker-free native-provider proof remain outstanding. See the [C01 contract](docs/bazel-test-harness.md#c01-compose-service).
