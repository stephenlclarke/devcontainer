...
The [final delivery contract](docs/bazel-workflow.md#final-delivery-and-public-evidence) includes stable releases of both projects, a full published-binary test cycle, public quiet-host benchmark evidence, both DocC sites, and installation-first live demos. The stock E01 protocol check now passes after explicit helper approvals. Configuration-digest lookup and descriptor-bound creation have source fixes and focused tests, but E02 still needs candidate/live qualification and the new transport path needs more coverage. Neither result replaces the certified historical release matrix or establishes new full parity.
...
`make bazel-prepare-docker-oracle` prepares the pinned published Colima/Lima tools and compressed Docker VM image through that same path; `OFFLINE=1` supports verified reuse. It does not start a VM, change Docker contexts or touch existing Colima profiles. Docker CLI acquisition, isolated VM lifecycle/recovery and live oracle qualification remain unfinished. Docker is a test reference only, not a product installation dependency.
