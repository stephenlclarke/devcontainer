...
Native process input cancellation now interrupts a full socket or terminal queue; descriptor closure remains on its owning worker. CLI-backed and PTY output monitors drain nonblocking descriptors and finish cancellation on their reader; the separate direct-API output monitor is unchanged. This does not implement the still-missing prepared Compose foreground attach path. Native Bazel coverage gates reject malformed counters and coverage-merger diagnostics, even when Bazel reports successful tests. The current full native instrumented suite has an unresolved counter-underflow diagnostic and cannot certify release coverage; see [current cancellation and coverage evidence](docs/bazel-test-harness.md#process-io-cancellation-and-coverage-integrity).
