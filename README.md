...
The Bazel E14 fixture now tests the guest's first terminal-size query through a real host PTY, followed by exact merged output, terminal input, exit status and removal. Component tests pass; live Docker/native terminal-size qualification remains pending. See the [foreground evidence and limits](docs/bazel-test-harness.md#foreground-init-attachment-development).
