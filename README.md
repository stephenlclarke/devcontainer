...
The unreleased init-attachment path connects input/output before native startup, preserves independent clients and real exit status, and joins cleanup before replacing a process generation. The HTTP adapter applies stream selection, stdin EOF policy, TTY detach keys, bounded output and generation-checked post-start resize. An acknowledged native exit registration now survives auto-removal independently of output drainage; the matching Compose foreground client is component-tested. Native source-aware log history and full foreground live parity remain unfinished. [Runtime and HTTP evidence](docs/bazel-test-harness.md#foreground-init-attachment-development) distinguishes component tests from live parity and release gates. Invalid compiler coverage counters still prevent release-quality certification.
