...
D05 locked Features is in development: the new Bazel adapters require the original pinned installations and a specific missing-frozen-lock error, not any failed command. Component tests pass; live reference/native qualification is still pending. See the [D05 contract](docs/bazel-test-harness.md#d05-locked-features). This does not change the stable release's support claims.
